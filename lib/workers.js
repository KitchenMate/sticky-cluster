...
module.exports = function (options) {
  ...
  function serve (server) {
    ...
    process.on('message', function (message, connection) {
      ...
      if (message === PREFIX + 'connection') {
        ...
        // log
        if (DEBUG) log('WORKER #%d  got conn from %s', id, connection.headers['x-forwarded-for'] || connection.remoteAddress);
  ...
  function entrust (index, connection) {
    var id = ids[index];
    if (DEBUG) log('MASTER  conn from %s goes to worker #%d', connection.headers['x-forwarded-for'] || connection.remoteAddress, id);
